...
// 重置fd上的事件，这样操作以后，尽管fd上的EPOLLONESHOT事件被注册，但是操作系统仍然会触发EPOLLIN事件，且只触发一次
...
void *worker(void* arg)
{
    ...
    printf("end thread receiving data of fd: %d\n", sockfd);
}
...
int main(int argc, char *argv[])
{
    ...
    assert(epollfd != -1);
    // 注意，监听socket listenfd上不能注册EPOLLOINESHOT事件的，
    // 否则应用程序只能处理一个客户连接。
    ...
    while (1)
    {
        ...
        for (int i = 0; i < ret; ++i)
        {
            ...
            if (sockfd == listenfd)
            {
                ...
                // 对每个非监听，描述符都注册EPOLLONESHOT
                addfd(epollfd, connfd, true);// 使用EPOLLONESHOT
            }
